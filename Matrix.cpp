...
void memset(Matrix& m, const size_t rows, const size_t cols, const float data)
{
	...
	for (register size_t i = 0; i < m.__rows * m.__cols; ++i)
		m.__data[i] = data;
	if (rows && cols)
		*m.__refcount = 1;
	else
...
bool operator==(const Matrix& m1, const Matrix& m2)
{
	if (m1.__rows == m2.__rows && m1.__cols == m2.__cols)
		for (register size_t i = 0; i < m1.__rows * m1.__cols; ++i)
...
Matrix operator*(const float n, const Matrix& m)
{
	...
	if (*m.__refcount)
		for (register size_t i = 0; i < tmp.__rows * tmp.__cols; ++i)
			tmp.__data[i] *= n;
	else std::cerr << "The multiplier is an empty matrix!" << std::endl;
	return tmp;
}
